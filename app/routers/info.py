...
async def get_customers(
    ...
    # 고객 목록 조회
    customers: List[Customer] = await Customer.filter(
    ...
    return JSONResponse({
        ...
        "data": customers,  # `items` 속성은 `list` 타입입니다.
    })
...
async def get_customer(
    ...
    # 고객 정보 조회
    customer: Customer = await Customer.get(customer_id)  # `customer` 변수는 `Customer` 타입입니다.
